...
import MultiKEDMM.TopicalWordComparator;
...
public class MultiKE_DMM {
	...
	public MultiKE_DMM(ArrayList<Document> doc_list, int num_topic, int num_iter, double beta,
	...
	public boolean loadWordMap(String filename) {
		try {
			...
			while ((line = reader.readLine()) != null) {
				line = line.trim();
				word2id.put(line, index);
				id2word.put(index, line);
				index++;
			}
...
//		int randomSeed = 47;
		...
		for (int i = 0; i < docList.size(); i++) {
			...
			for (int j = 0; j < doc.words.length; j++) {
				termIDArray[j] = Integer.parseInt(doc.words[j]);
				docWordGPUFlag.add(false); // initial for False for every word
				docWordGPUInfo.add(new HashMap<Integer, Double>());
				int times = 0;
				if(wordOccurenceToIndexInDocCount.containsKey(termIDArray[j])) {
					times = wordOccurenceToIndexInDocCount.get(termIDArray[j]);
				}
				times+=1;
				wordOccurenceToIndexInDocCount.put(termIDArray[j], times);
...
//			int topic = assignmentList[d];
		...
		for (int iteration = 1; iteration <= numIter; iteration++) {
			...
			for (int s = 0; s < docToWordIDList.size(); s++) {
				...
				for (int topic = 0; topic < numTopic; topic++) {
					...
					for (int t = 0; t < termIDArray.length; t++) {
						int termID = termIDArray[t];
						value *= (nzw[topic][termID] + beta+occurenceToIndexCount.get(s).get(t)-1) / (nz[topic] + vocSize * beta + t);						
					}
					value = pz * value;
					pzDist[topic] = value;
				}
	...
	public void saveModel(String flag) {
		...
		saveModelTopWordInEachTopic(flag+"_TopWORD.txt");
		
		saveModelWords(flag + "_words.txt");
		saveModelAssign(flag + "_assign.txt");
		
		saveModelPz(flag + "_pz.txt");
		saveModelPhi(flag + "_pzw.txt");
	...
	public boolean saveModelTopWordInEachTopic(String filename) {
...
//					sb.append(id2word.get(max_index)+"\t");
					sb.append(max_index+" ");
					phi_t[max_index] = 0;
...
//			System.out.print(sb.toString());
			...
			System.out.println("Error while saving saveModelTopWordInTopick: " + e.getMessage());
			e.printStackTrace();
			return false;
		}
		return true;
